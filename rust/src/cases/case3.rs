...
pub fn case3() -> Universe {
    ...
    let star_spin0 = TWO_PI/(star_rotation_period/24.); // days^-1
    let star_spin = Axes{x:0., y:0., z:star_spin0 };
    ...
    let planet_spin0 = TWO_PI/(planet_rotation_period/24.); // days^-1
    let keplerian_orbital_elements = calculate_keplerian_orbital_elements(G*star_mass*planet_mass, planet_position, planet_velocity);
