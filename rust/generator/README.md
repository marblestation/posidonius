...
# Posidonius generator

The Posidonius generator is the python package used to design simulations to be used with Posidonius.


## Installation

The package can be installed system wise or only for the current user:

```bash
python setup.py install --user
```

The `--user` flag will install the package in your `$HOME/.local/lib/python2.7/site-packages/`. To uninstall:
...
The generator can be used as shown in the following example:
...
```bash
mkdir -p output/
python example.py output/example.json
```

The script will create a json file with the simulation description needed by Posidonius.
